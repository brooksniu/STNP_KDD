...
x_train = np.load('x_train_initial.npy')
y_train = np.load('y_train_initial.npy')
...
#     print("y_pred_shape: ", y_pred_test.shape)
    ...
    test_mae = MAE(torch.from_numpy(y_pred_test).float(),torch.from_numpy(y_test).float()) / scale_std_y
...
#     print("shape of y_pred_all: ", y_pred_all.shape)
    ...
    for k in range(int(len(search_data_x))):
        index = np.random.choice(len(search_data_x), 10, replace=False).tolist()
...
#         print("shape of search_data_y_all: ", search_data_y_all.shape)
    ...
    torch.save(dcrnn.state_dict(), 'lig_seed%d_itr%d.pt' % (seed, i+1))
...
# train_losses, val_losses, test_losses, z_mu, z_logvar = train(5000,x_all,y_all,x_val, y_val, x_test, y_test,500, 1500) #20000, 5000
